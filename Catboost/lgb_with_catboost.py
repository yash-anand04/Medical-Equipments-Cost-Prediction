# transport_cost_ensemble_robust.py
...
# Fill NaNs in object columns with a "Missing" category
...
# --- MODIFIED: Dynamic Category Detection ---
# Dynamically find all object/string columns that exist
cat_features_list = combined.select_dtypes(include=['object', 'category']).columns.tolist()

# Manually add features we know are categorical (even if they are numeric)
manual_cat_features = ["Hospital_Id", "Order_Month", "Order_DayOfWeek", "Order_Is_Weekend"]
for col in manual_cat_features:
    if col not in cat_features_list and col in combined.columns:
        cat_features_list.append(col)
        
print(f"Discovered and defined {len(cat_features_list)} categorical features: {cat_features_list}")
# --- END MODIFICATION ---
...
# For CatBoost, ensure all categorical features are treated as strings
...
lgbm_cat_features = [] # This will be the list of encoded column *names*

for col in cat_features_list:
    if col in combined_lgbm.columns:
        combined_lgbm[col] = combined_lgbm[col].astype(str) # Ensure all are strings before encoding
        le = LabelEncoder()
        combined_lgbm[col] = le.fit_transform(combined_lgbm[col])
        lgbm_cat_features.append(col) # Add to list for LGBM
...
for fold, (train_index, val_index) in enumerate(kf.split(X_processed_lgbm, y_log)):
    ...
    model_lgbm.fit(X_train_lgbm, y_train_fold,
                   ...
                   categorical_feature=lgbm_cat_features) # Use the list of encoded column names
    ...
    model_catboost.fit(X_train_cat, y_train_fold,
                       ...
                       cat_features=cat_features_list, # Pass the *names* of the categorical features
                       use_best_model=True
                      )
...
# ✅ Save safely
output_path = "submission_ensemble_robust.csv" # New name
