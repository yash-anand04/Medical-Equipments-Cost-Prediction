...
# --- ⭐️ NEW FIX HERE ⭐️ ---
# Fill NaN values in the target variable *before* any processing
print(f"Original missing Transport_Cost values: {train['Transport_Cost'].isnull().sum()}")
cost_median = train['Transport_Cost'].median()
train['Transport_Cost'] = train['Transport_Cost'].fillna(cost_median)
print(f"Missing values after fill: {train['Transport_Cost'].isnull().sum()}")
# --- ⭐️ END FIX ⭐️ ---
...
# Log-transform the target (now safe from NaNs)
...
# Dynamic Category Detection (using your print log as a guide)
...
# ✅ Save safely
output_path = "submission_ensemble_final.csv" # New name
