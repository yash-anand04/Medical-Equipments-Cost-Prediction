# transport_cost_3model_ensemble.py
...
from sklearn.linear_model import ElasticNet # --- NEW ---
...
# --- Prep for LGBM (Label Encoded) ---
print("Label Encoding for LightGBM & Linear...")
...
# --- Get list of all columns that are *not* categorical for scaling ---
numeric_cols = [col for col in X_processed_encoded.columns if col not in lgbm_cat_features and col != 'Hospital_Id_encoded']
print(f"Found {len(numeric_cols)} numeric features for scaling: {numeric_cols}")
# --- End Data Prep ---
...
test_preds_elastic_list = [] # --- NEW ---
oof_maes_lgbm = []
oof_maes_catboost = []
oof_maes_elastic = [] # --- NEW ---
...
for fold, (train_index, val_index) in enumerate(kf.split(X_processed_encoded, y_log, groups=groups)):
    ...
    # --- 1. LightGBM (Uses Label Encoded Data) ---
    print("Training LGBM...")
    X_train_lgbm, X_val_lgbm = X_processed_encoded.iloc[train_index], X_processed_encoded.iloc[val_index]
    model_lgbm = LGBMRegressor(objective='regression_l1', n_estimators=5000, learning_rate=0.01, num_leaves=64,
    ...
    model_lgbm.fit(X_train_lgbm, y_train_fold, eval_set=[(X_val_lgbm, y_val_fold)],
                   eval_metric='mae', callbacks=[early_stopping(200, verbose=False)],
    ...
    # --- 2. CatBoost (Uses String Data) ---
    print("Training CatBoost...")
    X_train_cat, X_val_cat = X_processed_catboost.iloc[train_index], X_processed_catboost.iloc[val_index]
    model_catboost = CatBoostRegressor(iterations=5000, learning_rate=0.01, loss_function='MAE', eval_metric='MAE',
                                       random_seed=42, verbose=0, early_stopping_rounds=200, l2_leaf_reg=3)
    model_catboost.fit(X_train_cat, y_train_fold, eval_set=[(X_val_cat, y_val_fold)],
                       cat_features=cat_features_list, use_best_model=True)
    ...
    # --- 3. ElasticNet (Uses Scaled, Label Encoded Data) ---
    ...
    # ElasticNet is not sensitive to MAE objective, it just fits
    model_elastic = ElasticNet(alpha=0.1, l1_ratio=0.5, random_state=42) # You can tune alpha
    model_elastic.fit(X_train_enc, y_train_fold)
    
    val_preds_elastic = np.expm1(model_elastic.predict(X_val_enc))
    fold_mae_elastic = mean_absolute_error(y_val_fold_orig, val_preds_elastic)
    print(f"ElasticNet Fold {fold+1} MAE: {fold_mae_elastic:.2f}")
    oof_maes_elastic.append(fold_mae_elastic)
    test_preds_elastic_list.append(np.expm1(model_elastic.predict(X_test_enc)))
    # --- END NEW MODEL ---
...
print(f"Overall OOF ElasticNet MAE: {np.mean(oof_maes_elastic):.2f} +/- {np.std(oof_maes_elastic):.2f}")
...
final_test_preds_elastic = np.mean(test_preds_elastic_list, axis=0)

# We'll give 1/3 weight to each model
final_test_predictions = (final_test_preds_lgbm + final_test_preds_catboost + final_test_preds_elastic) / 3.0
...
# ✅ Save safely
output_path = "submission_3model_ensemble.csv" # New name
