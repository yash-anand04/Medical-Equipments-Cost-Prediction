...
from sklearn.linear_model import LassoCV # --- Using your Lasso idea! ---
...
test_preds_lasso_list = [] # --- MODIFIED ---
oof_maes_lgbm = []
oof_maes_catboost = []
oof_maes_lasso = [] # --- MODIFIED ---
...
for fold, (train_index, val_index) in enumerate(kf.split(X_processed_encoded, y_log, groups=groups)):
    ...
    # --- 3. LassoCV (Uses Scaled, Label Encoded Data) ---
    ...
    # --- MODIFIED: Use LassoCV ---
    # LassoCV automatically finds the best alpha (hyperparameter tuning)
    model_lasso = LassoCV(cv=5, random_state=42, n_jobs=-1, max_iter=2000)
    model_lasso.fit(X_train_enc, y_train_fold)
    
    print(f"Lasso Fold {fold+1} Best Alpha: {model_lasso.alpha_:.6f}")
    
    val_preds_lasso = np.expm1(model_lasso.predict(X_val_enc))
    # Clip any potential negative predictions from linear model
...
# --- MODIFIED: Weighted Ensemble ---
# Give more weight to the linear model, as per your friend's success
...
# ✅ Save safely
output_path = "submission_lasso_ensemble.csv" # New name
