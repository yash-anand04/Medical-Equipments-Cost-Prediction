...
# --- ⭐️ NEW THOUGHT PROCESS: REMOVE EXTREME OUTLIERS ⭐️ ---
# That 35k MAE fold is caused by outliers. Let's remove them.
# We'll calculate the 99.5th percentile and treat anything above it as un-learnable noise.
PERCENTILE_CLIP = 0.995 # You can tune this (e.g., 0.99)
...
# This now uses the *filtered* training set
...
# We concat the *filtered* X and the *full* test set
...
# --- Prep for CatBoost ---
# IMPORTANT: We must re-split using the *new* length of X
...
combined_lgbm['Hospital_Id_encoded'] = le_hospital.fit_transform(combined_lgbm['Hospital_Id'].astype(str))
for col in cat_features_list:
...
X_processed_lgbm = combined_lgbm.iloc[:len(X), :]
X_test_processed_lgbm = combined_lgbm.iloc[len(X):, :]
# --- End Data Prep ---
...
# Get the groups from the *new* filtered dataframe
groups = X_processed_lgbm['Hospital_Id_encoded'] 
...
# Use the new filtered X and y_log for splitting
for fold, (train_index, val_index) in enumerate(kf.split(X_processed_lgbm, y_log, groups=groups)):
    print(f"\n--- Fold {fold+1}/{N_SPLITS} ---")
    y_train_fold, y_val_fold = y_log.iloc[train_index], y_log.iloc[val_index]
    y_val_fold_orig = y.iloc[val_index] # This 'y' is already filtered
...
# ✅ Save safely
output_path = "submission_ensemble_outliers_removed.csv" # New name
