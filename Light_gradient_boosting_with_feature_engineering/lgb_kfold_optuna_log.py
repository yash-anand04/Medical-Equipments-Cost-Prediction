# transport_cost_tuned_TEST.py

import pandas as pd
import numpy as np
import optuna
import optuna.logging  # --- NEW: Added for progress logging ---
optuna.logging.set_verbosity(optuna.logging.INFO) # --- NEW: This makes Optuna print its progress ---
...
test_df_orig = test_df.copy() # Store original test set for submission
...
combined["Order_DayOfMonth"] = combined["Order_Placed_Date"].dt.day # --- FIX: Was .dayofmonth ---
combined["Order_Quarter"] = combined["Order_Placed_Date"].dt.quarter
combined["Order_Is_Weekend"] = (combined["Order_Placed_Date"].dt.dayofweek >= 5).astype(int)
combined["Order_Is_Month_End"] = combined["Order_Placed_Date"].dt.is_month_end.astype(int)
...
def objective(trial, X, y_log, y_orig):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X, y_log)):
        ...
        y_val_fold_orig = y_orig.iloc[val_index]
...
print("Starting Optuna hyperparameter search (FAST TEST RUN)...")
study = optuna.create_study(direction='minimize')

# --- MODIFIED: Set n_trials to 10 for a fast test ---
study.optimize(lambda trial: objective(trial, X_processed, y_log, y), n_trials=10)
...
for fold, (train_index, val_index) in enumerate(kf.split(X_processed, y_log)):
    ...
    model.fit(X_train_fold, y_train_fold,
              eval_set=[(X_val_fold, y_val_fold)],
              eval_metric='mae',
              callbacks=[early_stopping(100, verbose=False)], # Set to True if you want to see fold details
              categorical_feature=lgbm_cat_features)
...
output_path = "submission_lgbm_tuned_TEST.csv"
